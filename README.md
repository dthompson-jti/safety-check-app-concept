...
-   **Zero-CLS Offline Indicator:** When offline, the header smoothly expands (+6px) while status badges shift up and an offline pill slides in below them. Menu and avatar remain stationary. The pill displays "Offline: MM:SS — N Queued" with coordinated state transitions: instant exit (0.05s) + slow enter (0.4s) for smooth morphing. Reconnection shows a green "Connected" state with success pulse animation (matching CheckCard completion), followed by background fade and outward glow.
